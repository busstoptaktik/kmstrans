...
class BshlmWidget(WidgetBase, Ui_tab_bshlm):
    ...
    def handleAngularUnitChange(self, geo_unit):
        ...
        if is_mode1 and self.cache.is_valid and self.cache.mode == 0:
            self.txt_bshlm_azimuth1.setText(
                translateFromDegrees(self.cache.a1, geo_unit, precision=1))
            self.txt_bshlm_azimuth2.setText(
                translateFromDegrees(self.cache.a2, geo_unit, precision=1))
        else:
