...
class PythonWidget(WidgetBase,Ui_tab_python):
	def __init__(self,parent):
		...
		self.logPython("Handle to main window %s stored in name: mainWindow" %repr(parent.__class__),color="brown")
		self.python_console=EmbedPython.PythonConsole(namespace)
		self.python_console.executeCode("from TrLib import *")
		self.logPython("from TrLib import *  - Loading namespace from TrLib.py",color="brown")
		...
		if not HAS_QSCI:
			self.logPython("Qscintilla not installed. Python input lexer will not work...",color="red")
	...
	def onPythonCommand(self):
		...
		if cmd.lower()=="clear":
			...
			self.logPython("Interactive python help is not available in this mode...",color="red")
