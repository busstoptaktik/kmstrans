...
# is_angle determines whether input should be considered angles and thus
# translated - z_fields is a list of those fields that are NOT angles even
# if some other fields are.
...
def setOutput(coords, fields, is_angle=False, z_fields=[2], angular_unit="dg", precision=4):
    ...
    for i in range(len(fields)):
        if is_angle and (not i in z_fields):
            fields[i].setText("%s" % (translateFromDegrees(
                coords[i], angular_unit, precision=precision)))
        else:
...
def translateAngularField(field, geo_unit, precision=4):
    ...
    field.setText("%s" % translateFromDegrees(
        ang, geo_unit, precision=precision))
