...
# Thus - try to comply with the Qt4 style for methods: lowerUpper. See
# also: https://code.google.com/p/soc/wiki/PythonStyleGuide
...
PLUGIN_PATH_USER = os.path.expanduser(
    os.path.join("~", "." + PROG_NAME, "plugins"))
...
if "HOME" in os.environ:
    # encoding can be problematic - cant assume it's ascii... thus use unicode
    # conversion on saved Qt4 strings.
...
class MapEvent(QEvent):
    # Signal that map rendering has finished
...
class FileLogEvent(QEvent):
    # To send logging events during a background process.
...
class ReturnCodeEvent(QEvent):
    # Signal that a background process has finished with some kind of code.
...
class MessagePoster(object):
    """
    Post an event to a window. To be used by a regular python Thread doing some kind of background work.
    """
...
class MapThread(threading.Thread):
    # Would be more Qt'ish to subclass QtThread and use its postEvent method. Then we wouldn't need to keep the app as a global.
    # However the app approach works fine also for the MessagePoster which is
    # used in a normal python Thread (shouldn't need to know anything about
    # Qt).

    def __init__(self, win, region=REGION_DK):
        threading.Thread.__init__(self)
        self.win = win
        # not used at the moment - for selecting coastline according to
        # region....
        self.region = region
...
# Get a list of plugins - now from several paths. Enables plugins in root
# dir also
...
class PointData(object):
    """Keep cached output from a transformation"""
    ...
    def __init__(self):
        ...
        self.scale = None
        # flag for type 8 and 10 systems - right now detected on minilabel
        # level. Would be great to expose the proj object (and thereby cstm
        # code etc.)
...
def getNumericScale(x1, y1, coord_transf, axis, flat):
    ...
    if (d is not None):
        ...
        print("Exception during calculation of scale and convergence.")
        sc, m = -1, -1
    return sc, m
...
class DialogGDALSettings(QtGui.QDialog, Ui_Dialog_gdal):
    """
    Dialog for setting up GDAL installation to use.
    """
    ...
    def __init__(self, parent, settings):
        ...
        self.setupUi(self)
        self.mode_buttons = [self.rdb_gdal_system,
                             self.rdb_gdal_included, self.rdb_gdal_custom]
        self.txt_paths = [self.txt_bin_folder,
                          self.txt_data_folder, self.txt_plugin_folder]
        self.browse_buttons = [self.bt_browse_bin,
                               self.bt_browse_data, self.bt_browse_plugin]
    ...
    def onModeChange(self):
        enable = not (self.rdb_gdal_system.isChecked()
                      or self.rdb_gdal_included.isChecked())
        for field in self.txt_paths:
            field.setEnabled(enable)
        for bt in self.browse_buttons:
            bt.setEnabled(enable)
        if self.rdb_gdal_system.isChecked():
            # or ctypes.util.find_library("gdal")
            self.txt_paths[0].setText("")
            i = 1
    ...
    def on_bt_browse_bin_clicked(self):
        my_file = unicode(QFileDialog.getExistingDirectory(
            self, "Select a directory contaning gdal binaries.", self.dir))
        if len(my_file) > 0:
    ...
    def on_bt_browse_data_clicked(self):
        my_file = unicode(QFileDialog.getExistingDirectory(
            self, "Select a directory containing gdal data files.", self.dir))
        if len(my_file) > 0:
    ...
    def on_bt_browse_plugin_clicked(self):
        my_file = unicode(QFileDialog.getExistingDirectory(
            self, "Select a directory containing gdal plugins.", self.dir))
        if len(my_file) > 0:
    ...
    def on_bt_apply_clicked(self):
        ...
        if changes:
            ...
            QMessageBox.information(
                self, "GDAL settings", "Changes will take effect after a restart.")
        self.accept()
...
class AffineModifications(object):
    """Holds information about the affine modifications of input and output."""
...
class AffineWidget(QtGui.QWidget, Ui_Widget_affine):
    ...
    def __init__(self, parent, title, params):
        ...
        self.R = [[self.r11, self.r12, self.r13], [
        ...
        for i in range(3):
            ...
            if len(v) > 0:
                try:
                    ...
                    self.T[i].setFocus()
                    QMessageBox.warning(
                        self, "Error", "Specify a numeric input")
                    return None
                if v != 0.0:
                    vals[i] = v
        return vals
...
class DialogSetupAffine(QtGui.QDialog, Ui_Dialog_affine):
    ...
    def __init__(self, parent, params):
        ...
        # self.affine_in.chb_apply.toggled.connect(self.onEnableAffine) #check
        # event propagation!
    ...
    def on_bt_apply_clicked(self):
        ...
        # and non-trivial mods which are not applied should also trigger
        # message.
        nvals = 0
        ...
        for widget, name in [(self.affine_in, "input"), (self.affine_out, "output")]:
            ...
            if do_apply:  # only save if apply is checked...
                ...
                if T is None:
                    return
                # TODO: handle the situatuon where one the transormations is
                # trivial...
                vals_here = len(R) + len(T)
                if vals_here == 0:
                    msg = "Affine modification enabled in " + \
                        name + "- but no nontrivial values set."
        ...
        if (self.chb_apply_f2f.isChecked() or self.chb_apply_interactive.isChecked()):
            if nvals == 0:
                QMessageBox.information(
                    self, "Affine setup", "No nontrivial modifications to apply. Cancel or set some!")
                return
        else:
            if nvals > 0:
                QMessageBox.information(
                    self, "Affine setup", "You have enabled affine modificaitions, but they are not applied in any tab.")
                return
        # in a consistent state and ready to fly, so modify stored params.
        self.params.apply_f2f = bool(self.chb_apply_f2f.isChecked())
        self.params.apply_interactive = bool(
            self.chb_apply_interactive.isChecked())
...
class DialogFile2FileSettings(QtGui.QDialog, Ui_Dialog_f2f):
    ...
    def __init__(self, parent, settings):
        ...
        self.rdbs = [self.rdb_dg, self.rdb_sx, self.rdb_nt, self.rdb_rad,
                     self.rdb_dg_in, self.rdb_sx_in, self.rdb_nt_in, self.rdb_rad_in]
    ...
    # TODO: implement save and load settings like in MainWindow - should also
    # *always* display the current settings, so that the cancel button should
    # uncheck/recheck stuff.
    ...
    def apply(self):
        ...
        self.settings.kms_flip_xy_in = bool(
            self.chb_kms_flip_input.isChecked())
        self.settings.kms_flip_xy_out = bool(
            self.chb_kms_flip_output.isChecked())
...
class DialogCreationOptions(QDialog, Ui_Dialog_creation_options):
    ...
    def validate(self, text):
        ...
        for item in cops:
            if len(item.split("=")) != 2:
                self.message(
                    "Bad format of creation option %s, should be 'KEY=VALUE'" % item)
                return False
        return True
        ...
        if txt is not None:
            ...
            if not fok:
                ...
                if n / 1024 > 100:
                    maxlines = 1000
                    txt_field.append(
                        "***Large file - will only display the first %d lines:\n" % maxlines)
                else:
                    maxlines = None
                f = open(fname)
                nlines = 0
...
class LayerSelector(QDialog, Ui_Dialog_layer_selector):
    """Box for selecting multiple layernames to be transformed"""
...
class RedirectOutput(object):
    """
    Simple object which redirects stdout / stderr
    """
...
class LauncherWindow(QtGui.QMainWindow, Ui_LauncherWindow):
    """
    Launcher window in which geoid folder for TrLib can be selected and setup before launch of main window.
    """
    ...
    def __init__(self, parent=None):
        ...
        if not ok:
            ...
            self.log(
                "See https://bitbucket.org/KMS/kmstrans/wiki/Home for build instructions...", "blue")
            ...
            if self.geoids is None and "TR_TABDIR" in os.environ:
                ...
                self.log(
                    "Using geoid library defined in env var TR_TABDIR-", "green")
            if self.geoids is None:
                ...
                self.log("Geoid library currently set to: %s" %
                         self.geoids, "blue")
                ...
                if ok:
                    ...
                    self.log(
                        "Unable to initialise with current geoid library.", "red")
                    self.bt_launch.setEnabled(False)
                    self.show()
    ...
    def launch(self):
        ...
        # hmm this only closes the window - its not deleted. Seems to be
        # sufficient for the app to shut down though...
        self.close()
    ...
    def on_bt_geoids_clicked(self):
        gdir = unicode(QFileDialog.getExistingDirectory(
            self, "Select a valid geoid directory", "/"))
        if (gdir is None or len(gdir) == 0):
            return
        for name in REQUIRED_FILES:
            if not os.path.exists(os.path.join(gdir, name)):
                self.log("Required file %s not found in %s" %
                         (name, gdir), "red")
                return
        self.geoids = gdir
        ok = self.setGeoidLibrary()
        if ok:
            self.log(
                "Successfull initialisation using geoid library from %s" % gdir, "blue")
...
class TRUI(QtGui.QMainWindow, Ui_Trui):
    ...
    def __init__(self, parent=None, geoids=None):
        ...
        self.setupUi(self)
        self.geoids = geoids
        # flag which signals whether to save settings - only do it if
        # initialisation succeded!
        self._save_settings = False
        # flag to signal whether to clear interactive log before each
        # transformation
        ...
        self.cb_input_system.currentIndexChanged.connect(
            self.onSystemInChanged)
        self.cb_output_system.currentIndexChanged.connect(
            self.onSystemOutChanged)
        self.cb_f2f_input_system.currentIndexChanged.connect(
            self.onF2FSystemInChanged)
        self.cb_f2f_output_system.currentIndexChanged.connect(
        ...
        self.actionDegrees_derived.triggered.connect(
            self.setDerivedAngularUnitsDegrees)
        self.actionRadians_derived.triggered.connect(
            self.setDerivedAngularUnitsRadians)
        ...
        self.actionFile2file_settings.triggered.connect(
            self.openFile2FileSettings)
        ...
        self.actionSetup_affine_modifications.triggered.connect(
            self.setupAffine)
        ...
        # +self.output_bshlm_azimuth[1:]
        self.derived_angular_output = [self.txt_meridian_convergence]
        ...
        self.setDerivedAngularUnitsDegrees()
        # will also be set in loadSettings - TODO: save and load other
        # settings...
        ...
        if IS_WINDOWS:
            # this is also needed in order to fix the library dependecies in
            # the bin subfolder#
            ...
            if (load_mode == 1 or load_mode == 2):
                if load_mode == 1:
                    ...
                    paths = [p.encode(sys.getfilesystemencoding())
                             for p in self.gdal_settings.paths]  # unicode objects
        ...
        self.loadLibtrui()
        # important to set this var which will be passed on to trogr when
        # running batch mode
        os.environ["TR_TABDIR"] = self.geoids.encode(
            sys.getfilesystemencoding())
        # Now that trlib is initlialised we can define these tranformation
        # objects#
        ...
        # Only now - redirect python stderr - to be able to see errors in the
        # initialisation#
        self.initRegion()
        sys.stderr = RedirectOutput(self.handleStdErr)
        # flag which signals whether to save settings - only do it if
        # initialisation succeded - so now it's ok!
    ...
    def initTransformations(self):
        self.numeric_scale_transf = TrLib.CoordinateTransformation(
            "", "geo_etrs89")
        self.fallback_ellipsoid = TrLib.GetEllipsoidParametersFromDatum(
            "etrs89")
        self.coordinate_transformation = TrLib.CoordinateTransformation("", "")
        #Initialse the map transformation#
        self.map_transformation = TrLib.CoordinateTransformation(
            "geo_wgs84", "geo_wgs84")
    ...
    def initMap(self):
        ...
        # always add it - then we can remove and add again later
    ...
    def loadLibtrui(self):
        ...
        # and perhaps in the 'finished event handler we should zoom to the
        # point??
    ...
    def onRdobtOGRToggled(self):
        ...
        if (not checked):
            ...
            self.logF2F(
                "Note: If your vector datasource is not 3-dimensional you should toglle off height in your input and output systems.", "blue")
    ...
    def on_bt_f2f_view_output_clicked(self):
        ...
        if len(self.f2f_settings.output_files) > 1:
            fname, ok = QInputDialog.getItem(
                self, "Ouput files", "Select a file to view", self.f2f_settings.output_files, False)
        else:
    ...
    def on_bt_f2f_change_h_in_clicked(self):
        ...
        mlb = Minilabel.changeHeightSystem(
            mlb_in, H_SYSTEMS[self.region], DATUM_ALLOWED_H_SYSTEMS)
        if mlb != mlb_in:
    ...
    def on_bt_f2f_browse_input_dir_clicked(self):
        my_file = QFileDialog.getExistingDirectory(
            self, "Select an input directory", self.dir)
        if len(my_file) > 0:
            self.txt_f2f_input_file.setText(my_file)
            self.dir = os.path.dirname(unicode(my_file))
    ...
    def on_bt_f2f_browse_output_clicked(self):
        my_file = QFileDialog.getSaveFileName(
            self, "Select a vector-data output file", self.dir)
        if len(my_file) > 0:
    ...
    def on_bt_f2f_browse_output_dir_clicked(self):
        my_file = QFileDialog.getExistingDirectory(
            self, "Select an output directory", self.dir)
        if len(my_file) > 0:
            self.txt_f2f_output_file.setText(my_file)
            self.dir = os.path.dirname(unicode(my_file))
    ...
    def on_bt_f2f_browse_log_clicked(self):
        my_file = QFileDialog.getSaveFileName(
            self, "Select a log file", self.dir)
        if len(my_file) > 0:
    ...
    def on_bt_f2f_input_layers_clicked(self):
        ...
        if len(inname) > 0:
            # do encoding here? depends on what GDAL-bindings does.
            ds = LibTrui.open(inname.encode(sys.getfilesystemencoding()))
            if ds is not None:
    ...
    def setupAffine(self):
        ...
        if r == QDialog.Accepted:
            ...
            # Set system info is only called in interactive tab on changes in
            # real systems.
    ...
    def loadSettings(self):
        ...
        if not IS_WINDOWS:
            ...
            settings.beginGroup('gdal')
            self.gdal_settings.load_mode, ok = settings.value(
                'load_mode', 1).toInt()
            paths = []
    ...
    def selectTabDir(self):
        my_file = unicode(QFileDialog.getExistingDirectory(
            self, "Select a valid geoid directory", self.dir))
        return my_file
    ...
    def togglePluginsEnabled(self):
        ...
        if self.enable_plugins:
            ...
            self.actionPlugins_enabled.setText("Enable plugins")
        self.saveSettings()
        QMessageBox.information(self, "Plugins " + state,
    ...
    def loadPlugins(self):
        ...
        if len(plugins) > 0:
            if not HAS_IMPORTLIB:
                self.logInteractive(
                    "Plugin loader needs importlib (python version>=2.7)", color="red")
                return
    ...
    def addPluginWidget(self, plugin):
        ...
        if hasattr(plugin, "getName"):
            ...
            name = "some_plugin"
        self.logInteractive(
            "Widget type plugin - added as tab: %s" % name, color="brown")
...
def main():
    ...
    # Avoid Qt4 changing the locale for c stdio functions which we wan't to
