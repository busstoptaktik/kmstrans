...
int *cache_geoid_usage(struct mgde_str *tab_table);
...
/* Function to cache geoid usage before transformations!*/
int *cache_geoid_usage(struct mgde_str *tab_table){
    int g,*geoid_usage;
    struct gde_lab  *t_lab;
    geoid_usage=calloc(sizeof(int),tab_table->tab_max);
    if (!geoid_usage)
        return NULL;
    for (g = 0, t_lab = &(tab_table->table_u[0]);   g < tab_table->tab_max;  g++, t_lab++) { /* thanks KE!*/
        geoid_usage[g]=t_lab->used;
    }
    return geoid_usage;
}
...
void log_geoids(struct mgde_str *tab_table, int *cached){
    ...
    if (n_used==0)
        Report(REP_INFO,0,VERB_LOW,"    No table hits...");
}
...
{  
   ...
   if (n_opts<3){
    ...
    /* At this point we can cache the geoid usage, since that pointer is not changed by the later TR_Inserts!*/
    if (trf->geoid_pt){
       geoid_usage=cache_geoid_usage(trf->geoid_pt);
    }
    time ( &rawtime );
